...
### Added
- ユーザーから提供された参照画像に基づいたトランプカード表面レイアウトの精密な再現とリファイン
  - コーナーのインデックス文字（A, 2... J, Q, K）をカード内境界枠の「外側」に配置し、クラシックなカード配置レイアウトを再現
  - インナー境界枠（`.card-main-frame`）をカードの中央アクティブエリアのみを囲む構造へと変更
  - 絵札（J/Q/K）のイラストに、参照画像に適合する「赤（#c0392b）」「青（#2980b9）」「金（#f1c40f）」「黒（輪郭）」の伝統的な4色塗り分けのラインアートを導入
  - スキンの切り替えに応じて、絵札のカラー変数（`--court-red`, `--court-blue` など）がネオンカラー（ピンク/ネオンブルー）やロイヤルゴールドへ自動同期する機能を追加
  - コーナー文字の絶対座標配置（`top-left`, `bottom-right`）の導入により、インナー枠と数字の重なりを回避

## [v0.4.5] - 2026-06-13

### Added
- トランプ表面デザインの大幅な高級感（プレミアム化）リファイン
  - カード表面にリアルな「布目（リネンフィニッシュ）格子状エンボス加工」のCSSテクスチャを再現
  - カードのフチの内側に、ゴールドやネオン等各スキンに連動する極細インナーフレーム（飾り枠）を実装
  - 絵札（J/Q/K）のイラストを、シンプルすぎる描画から幾何学的かつエレガントで洗練されたロイヤル紋章風の極細ラインアートSVGに刷新
  - スペードA（As）の装飾スペードを、外周に月桂樹のリース点線リングを施したより優美で精巧なデザインへと進化
  - カードの数字（Rank）フォントをカジノカード風のエレガントなSerif系（Cinzel/Playfair Display）に変更（ネオン・サイバースキンではシャープなOutfitを維持）

### Changed
- 絵札の描画比率や背景の半透明度をスキンに応じて動的調整し、各テーマ（ロイヤルゴールドやネオンサイバーなど）に完全に調和する色彩へと調整
