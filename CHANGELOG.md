...
### Added
- 高級感あふれるカジノ仕様のポーカーテーブル（長円形フェルト）へのデザイン刷新
  - 外周に肉厚で立体感のあるダーク本革/ウッド調クッション（Padded Leather Rail）を実装
  - テーマスキン（クラシック、ネオン、ロイヤル、ヴィンテージ）に完全に調和したベッティングライン（インナーライン）の自動切り替え表示
  - フェルト中央に、各スキンに合わせた微細な透過文字のウォーターマークロゴ（`PREFLOP TRAINER`）を追加
  - テーブル全体の沈み込みを表現するディープインナーシャドウ効果の追加

### Changed
- テーブルのアスペクト比を 1.25 / 1 に調整し、よりリアルなポーカーの楕円形テーブルを表現
- 席の配置間隔やミニカードのレスポンシブな間隔を微調整し、新しいクッションに合わせたバランスへ統合
